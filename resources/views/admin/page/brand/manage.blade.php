							...
							<div class="col-sm-12">
								...
								<thead>
									<tr role="row">
										...
										<th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Position: activate to sort column ascending" style="width: 430.078px;">Brand Name</th>
								...
								<tbody>	
										@php $sl = 1; @endphp
											...
											<tr >
													<td>{{ $sl }}</td>
													<td >{{ $brand->name }}</td>
													<td> <image src="{{ asset('uploads/brand/'.$brand->image) }}" height="120" width="120"></image></td><br>
													<td>{{ $brand->slug}}</td>
													<td>{{ $brand->status}}</td>
													<td>
													<!--<a href="{{ route('edit.brand', $brand->id) }}"> <button  class="btn btn-info btn-sm">Edit</button> </a>-->
												    <a  href="{{ route('delete.brand', $brand->id) }}" > <button  class="btn btn-danger btn-sm">Delete</button> </a>
													</td>
												</tr>
												@php $sl++; @endphp
											@endforeach
