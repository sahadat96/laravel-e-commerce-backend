							...
							<div class="col-sm-12">
								...
								<thead>
									<tr role="row">
										...
										<th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Position: activate to sort column ascending" style="width: 430.078px;">Division</th>
								...
								<tbody>	
										@php $sl = 1; @endphp
											@if(count($show_division) > 0)
												@foreach($show_division as $show_divisions)
											   <tr >
													<td>{{ $sl }}</td>
													<td >{{ $show_divisions->division_name }}</td>
													<td >
												        <a href="{{ route('delete_division', $show_divisions->id) }}"> <button  class="btn btn-danger btn-sm">Delete</button> </a>
                                                   </td>
												</tr>
												@php $sl++; @endphp
											@endforeach
