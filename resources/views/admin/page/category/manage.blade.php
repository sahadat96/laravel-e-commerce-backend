								...
								<tbody>	
										@php $sl = 1; @endphp
											...
											<tr >
													...
													<td> <image src="{{ asset('uploads/category/'.$category->cat_image) }}" height="120" width="120"></image></td><br>
													<td>{{ $category->cat_slug}}</td>
													<td>{{ $category->status}}</td>
													<td>
													<a href="{{ route('edit.category', $category->id) }}"> <button  class="btn btn-info btn-sm">Edit</button> </a>
												    <a  href="{{ route('delete.category', $category->id) }}" > <button  class="btn btn-danger btn-sm">Delete</button> </a>
													</td>
												</tr>
												@php $sl++; @endphp
											@endforeach
